<div align="center">
  <img src="./assets/header.svg" width="100%" alt="Sunshine-1023 profile banner" />
</div>

<br />

<div align="center">
  <p>🌸 🌸 🌸 &nbsp; <b>🎨 初音画廊 · Miku Gallery</b> &nbsp; 🌸 🌸 🌸</p>
  <p>
    <img src="./assets/miku-diamond-01.png" width="17%" alt="Sakura Miku under a starry sky" />
    <img src="./assets/miku-diamond-02.png" width="17%" alt="Sakura Miku in a flower garden" />
    <img src="./assets/miku-diamond-03.png" width="17%" alt="Hatsune Miku in summer light" />
    <img src="./assets/miku-diamond-04.png" width="17%" alt="Sakura Miku by the sea" />
    <img src="./assets/miku-diamond-05.png" width="17%" alt="Sakura Miku among cherry blossoms" />
  </p>
  <p>🎵 &nbsp; 🎵 &nbsp; 🎵</p>
</div>

<br />

<div align="center">
  <img src="https://img.shields.io/badge/UNDERGRADUATE-STUDENT-39C5BB?style=for-the-badge&labelColor=263238" alt="Undergraduate student" />
  <img src="https://img.shields.io/badge/FOCUS-APPLIED_ML-FF8FA3?style=for-the-badge&labelColor=263238" alt="Applied machine learning" />
  <img src="https://img.shields.io/badge/INTEREST-RECOMMENDER_SYSTEMS-74D7D0?style=for-the-badge&labelColor=263238" alt="Recommender systems" />
</div>

<div align="center">
  <a href="https://github.com/Sunshine-1023?tab=followers">
    <img src="https://img.shields.io/github/followers/Sunshine-1023?style=flat-square&logo=github&label=Followers&color=39C5BB" alt="GitHub followers" />
  </a>
  <a href="https://github.com/Sunshine-1023?tab=repositories">
    <img src="https://img.shields.io/github/stars/Sunshine-1023?affiliations=OWNER&style=flat-square&logo=github&label=Stars&color=FF8FA3" alt="GitHub stars" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=Sunshine-1023&style=flat-square&color=39C5BB&label=Profile+views" alt="Profile views" />
</div>

<br />

<h2>About Me · 关于我</h2>

<h3>在读大学生 · 应用机器学习学习者 · 推荐系统探索者</h3>

- 🎓 目前是一名<b>在读大学生</b>，持续学习机器学习、推荐系统与全栈开发。
- 🧠 专注于<b>应用机器学习</b>，关注可靠评估、表格建模和深度学习实践。
- 🧩 喜欢把数据、模型与工程实现连接起来，构建可复现的实验和数据应用。
- 🌱 通过开源项目与 Kaggle 记录成长，也在探索从模型到产品的完整工作流。

<h3>代码助手</h3>

我喜欢借助 AI 完成头脑风暴、代码解释、调试和文档整理，让想法更快落地。

日常使用：<b>Codex、Claude Code、ChatGPT、Gemini</b>。

<p>
  <img src="https://img.shields.io/badge/Codex-412991?style=for-the-badge&amp;logo=openai&amp;logoColor=white" height="34" alt="Codex" />
  <img src="https://img.shields.io/badge/Claude_Code-D99675?style=for-the-badge&amp;logo=anthropic&amp;logoColor=white" height="34" alt="Claude Code" />
  <img src="https://img.shields.io/badge/ChatGPT-10A37F?style=for-the-badge&amp;logo=openai&amp;logoColor=white" height="34" alt="ChatGPT" />
  <img src="https://img.shields.io/badge/Gemini-4285F4?style=for-the-badge&amp;logo=googlegemini&amp;logoColor=white" height="34" alt="Gemini" />
</p>

<h2 align="center">LLM Ecosystem &amp; Tech Stack</h2>

<div align="center">
  <img src="./assets/tech-stack.svg" width="100%" alt="LLM ecosystem and technical stack" />
</div>

<h2 align="center">What I Build · 我能构建什么</h2>

<div align="center">
  <img src="./assets/field-guide.svg" width="100%" alt="What I build: capabilities and project practice" />
</div>

<h2 align="center">Featured Projects</h2>

<table>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/Sunshine-1023/Predicting-Smartphone-Addiction">Predicting Smartphone Addiction</a></h3>
      <p>面向 Kaggle 表格二分类的可复现机器学习工程，包含 CatBoost / LightGBM、分层 OOF、配置化实验、测试与 CI。</p>
      <p><code>Python</code> <code>CatBoost</code> <code>LightGBM</code> <code>ML Engineering</code></p>
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/Sunshine-1023/asarec-pratice">FashionRec Transformer</a></h3>
      <p>基于 H&amp;M 数据的时尚推荐系统实践，使用 SASRecF 与四路召回融合，并以 MAP@12 进行离线评估。</p>
      <p><code>Python</code> <code>SASRecF</code> <code>RecBole</code> <code>Recommendation</code></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/Sunshine-1023/Petroleum-Plant-Data-Management-System">Well Cost Management</a></h3>
      <p>前后端分离的数据管理系统，覆盖 REST API、事务、存储过程、触发器与业务查询。</p>
      <p><code>Vue 3</code> <code>Django REST</code> <code>MySQL</code> <code>JavaScript</code></p>
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/Sunshine-1023/ML-pratice">Machine Learning Practice</a></h3>
      <p>机器学习实践与实验集合，包含数据处理、模型训练、情感分析等练习。</p>
      <p><code>Python</code> <code>PyTorch</code> <code>Machine Learning</code></p>
    </td>
  </tr>
</table>

<h2 align="center">Contribution Garden</h2>

<div align="center">
  <img src="./profile-3d-contrib/profile-season-animate.svg" width="98%" alt="3D contribution calendar" />
</div>

<div align="center">
  <img src="./assets/footer.svg" width="100%" alt="Miku-themed footer decoration" />
</div>
